...
function Detroit() {
  ...
  const loadBridge = (zPosition) => {
    ...
    loader.load('/DETRIOT_SNOW.glb', (gltf) => {
  ...
  const spawnEntities = () => {
    ...
    if (Math.random() > 0.5) {
      // to spawn an obstacle if the last obstacle position is different
      ...
      // spawn a coin if the last coin position is different
  ...
  const bridgeStartingZPosition = -70; 
  const bridgeResetThreshold = 1; //
  function animate() {
    if (gameOver) return; // if the game is over
